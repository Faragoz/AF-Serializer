...
class LVObjectConverter(LVTypeConverter):
    """
    Converter for LabVIEW Object (LVClass) serialization and deserialization.

    Handles the flattened string representation of LabVIEW objects according to
    the format: [NumLevels][ClassName][VersionList][ClusterData]

    :ivar supported_codes: Tuple containing the type code for LV Objects (0x78)
    :type supported_codes: tuple
    :ivar supported_types: Tuple of supported Python types for LV Objects
    :type supported_types: tuple
    """
    supported_codes = (0x78,)  # Código hipotético para LVClass
    supported_types = (type('LVObject', (), {}),)  # Placeholder, ver abajo

    @classmethod
    def _deserialize(cls, info: DeserializationData) -> DeserializationResult:
        """
        Deserializes a flattened LabVIEW object from bytes.

        :param info: Deserialization context containing buffer and offsets
        :type info: DeserializationData
        :return: Deserialization result containing the reconstructed LVObject
        :rtype: DeserializationResult
        """
        offset_d = int(info.offset_d)
        buffer = info.buffer

        # 1. Parse NumLevels (U32)
        num_levels, offset_d = bytes2num(buffer, offset=offset_d, count=1, dtype=LVDtypes.u4)
        num_levels = int(num_levels)

        # If NumLevels is 0, this is a LabVIEW Object with no data
        if num_levels == 0:
            return DeserializationResult(
                scalar=LVObject(class_name=[], versions=[], data=[]),
                offset_d=offset_d,
        ...
        # 2. Parse ClassName
        class_name, offset_d = cls._parse_class_name(buffer, offset_d)

        # 3. Parse VersionList
        versions, offset_d = cls._parse_version_list(buffer, offset_d, num_levels)

        # 4. Parse ClusterData
        cluster_data, offset_d = cls._parse_cluster_data(buffer, offset_d, num_levels, info)

        # Create LVObject instance
        lv_object = LVObject(
    ...
    @classmethod
    def _parse_class_name(cls, buffer: bytes, offset: int) -> Tuple[List[str], int]:
        """
        Parses the ClassName section of a flattened LV Object.

        Format: [TotalLength:U8][PascalString1][PascalString2]...[0x00][Padding]

        :param buffer: Byte buffer containing the data
        :param offset: Current offset in the buffer
        :return: Tuple of (list of class name parts, new offset)
        """
        # Total length of ClassName (excluding this byte)
        total_length = int(buffer[offset])
        offset += 1

        start_offset = offset
        class_parts = []

        # Parse Pascal strings until we hit a zero-length string
        while True:
            str_len = int(buffer[offset])
            offset += 1
            ...
            # Read the string
            class_part = buffer[offset:offset + str_len].decode('utf-8')
            class_parts.append(class_part)
            offset += str_len

        # Calculate padding to align to multiple of 4
        bytes_read = offset - start_offset + 1  # +1 for total_length byte
        padding = (4 - (bytes_read % 4)) % 4
        offset += padding

        return class_parts, offset

    @classmethod
    def _parse_version_list(cls, buffer: bytes, offset: int, num_levels: int) -> Tuple[
        List[Tuple[int, int, int, int]], int]:
        """
        Parses the VersionList section of a flattened LV Object.

        Each version is 4 U16 values: [W][X][Y][Z] representing version W.X.Y.Z

        :param buffer: Byte buffer containing the data
        :param offset: Current offset in the buffer
        :param num_levels: Number of levels (versions) to parse
        :return: Tuple of (list of version tuples, new offset)
        """
        versions = []

        for _ in range(num_levels):
            version_parts, offset = bytes2num(buffer, offset=offset, count=4, dtype=LVDtypes.u2, scalar=False)
    ...
    @classmethod
    def _parse_cluster_data(cls, buffer: bytes, offset: int, num_levels: int,
                            info: DeserializationData) -> Tuple[List[Any], int]:
        """
        Parses the ClusterData section of a flattened LV Object.

        Each level has: [DataLength:U32][ClusterData if length > 0]
        Data is ordered from oldest ancestor to most derived class.

        :param buffer: Byte buffer containing the data
        :param offset: Current offset in the buffer
        :param num_levels: Number of levels to parse
        :param info: Deserialization context
        :return: Tuple of (list of cluster data, new offset)
        """
        cluster_data = []

        for _ in range(num_levels):
            # Read data length for this level
            data_length, offset = bytes2num(buffer, offset=offset, count=1, dtype=LVDtypes.u4)
            data_length = int(data_length)

            if data_length == 0:
                # Default data for this level
                cluster_data.append(None)
            else:
                # Parse the cluster data
                # We need to deserialize a cluster here
                cluster_end = offset + data_length

                # Create a temporary header for the cluster
                cluster_header = HeaderInfo(
                    code=0x50,  # Cluster code
                    offset_h=offset,
                    size=0,
                    start=offset,
                    name=None
                ...
                # Deserialize the cluster
                cluster_result = ClusterConverter.deserialize(
                    info.fork(header=cluster_header, offset_d=offset)
                )

                cluster_data.append(cluster_result.value)
                offset = cluster_end

                # Padding to multiple of 4
                padding = (4 - (data_length % 4)) % 4
                offset += padding

        return cluster_data, offset

    @classmethod
    def _serialize(cls, value: 'LVObject', info: SerializationData) -> SerializationResult:
        """
        Serializes a LabVIEW object to flattened string format.

        :param value: LVObject instance to serialize
        :param info: Serialization context
        :return: Serialization result with flattened data
        """
        buffer = b""

        # 1. Serialize NumLevels
        num_levels = len(value.class_name) if value.class_name else 0
        buffer += num2bytes(num_levels, dtype=LVDtypes.u4)

        if num_levels == 0:
            # Empty LabVIEW Object
        ...
        # 2. Serialize ClassName
        class_name_bytes = cls._serialize_class_name(value.class_name)
        buffer += class_name_bytes

        # 3. Serialize VersionList
        version_bytes = cls._serialize_version_list(value.versions)
        buffer += version_bytes

        # 4. Serialize ClusterData
        cluster_bytes = cls._serialize_cluster_data(value.data, info)
    ...
    @classmethod
    def _serialize_class_name(cls, class_parts: List[str]) -> bytes:
        """
        Serializes the ClassName section.

        :param class_parts: List of class name components (e.g., ["Actor Framework.lvlib", "Actor.lvclass"])
        :return: Serialized bytes
        """
        # Build the Pascal strings
        pascal_strings = b""
        for part in class_parts:
            part_bytes = part.encode('utf-8')
            pascal_strings += bytes([len(part_bytes)]) + part_bytes

        # Add terminating zero
        pascal_strings += b"\x00"

        # Calculate total length
        total_length = len(pascal_strings)

        # Add padding to multiple of 4
        padding = (4 - ((total_length + 1) % 4)) % 4  # +1 for length byte

        result = bytes([total_length]) + pascal_strings + (b"\x00" * padding)

        return result

    @classmethod
    def _serialize_version_list(cls, versions: List[Tuple[int, int, int, int]]) -> bytes:
        """
        Serializes the VersionList section.

        :param versions: List of version tuples (W, X, Y, Z)
        :return: Serialized bytes
        """
        buffer = b""
        ...
        for version in versions:
            for part in version:
                buffer += num2bytes(part, dtype=LVDtypes.u2)

        return buffer

    @classmethod
    def _serialize_cluster_data(cls, data_list: List[Any], info: SerializationData) -> bytes:
        """
        Serializes the ClusterData section.

        :param data_list: List of cluster data (None for default, or actual data)
        :param info: Serialization context
        :return: Serialized bytes
        """
        buffer = b""
        ...
        for data in data_list:
            if data is None:
                # Default data - zero length
                buffer += num2bytes(0, dtype=LVDtypes.u4)
            else:
                # Serialize the cluster
                if isinstance(data, tuple):
                    cluster_result = ClusterConverter.serialize(data, info.fork())
                else:
                    # Assume it's already serialized or handle appropriately
                    raise ValueError(f"Unsupported cluster data type: {type(data)}")

                cluster_bytes = cluster_result.flat_buffer()
                data_length = len(cluster_bytes)

                # Write length and data
                buffer += num2bytes(data_length, dtype=LVDtypes.u4)
                buffer += cluster_bytes
